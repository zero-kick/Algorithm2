...
public class Main {
    ...
    public static void turnStart(int player) {
        int size = qs[player].size();
        // 처음에 들고 있는 큐의 사이즈 만큼에다가, 해당 플레이어의 최대 확장 거리를 곱한 만큼만 탐색을 한다.
        // ex) 성이 2개이고, 최대 확장 거리가 2이면 2개의 성이 1회씩 확장 후 다시 거기에서 1회 더 확장이므로 총 4회이다.
        for(int ext = 0; ext < size * s[player]; ext++) {
